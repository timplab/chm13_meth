...
meth_bins <- meth_bins %>%
  ...
  mutate(color = case_when(z_score > 0 ~ "Accessible", 
                           TRUE ~ "Inaccessible")) 
meth_GC <- ggplot(meth_bins, aes(x = start/1e6, y= z_score, fill = color))+geom_bar(stat = "identity", position = "dodge")+ylim(-5,5)+labs(x="Genomic coordinates (Mb)", y="Accessibility Z-score")+theme_classic(base_size = 25)+theme(legend.position = "bottom")+xlim(111,113.2)
meth_GC
...
meth_CG <- ggplot(meth_bins, aes(x = (start/1e6), y= freq)) + labs(x="Genomic coordinates (Mb)", y="Methylation")+theme_classic(base_size = 25)+ylim(0,1)+geom_line(size =1)+xlim(111,113.2)
