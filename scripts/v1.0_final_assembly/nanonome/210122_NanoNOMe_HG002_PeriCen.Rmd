...
blocks <- genomeBlocks(BSgenome.HG002.chrX, chrs = seqnames(BSgenome.HG002.chrX), width = 200000)
...
start=(55922235-20000000)/1e6
end=(59189270+20000000)/1e6
gc_bins <- meth_bins %>%
  mutate(z_score = (freq - freqmean) / freqsd) %>%
  mutate(color = case_when(z_score > 0 ~ "Accessible", 
                           TRUE ~ "Inaccessible")) 
meth_GC <- ggplot(gc_bins, aes(x = start/1e6, y= z_score, fill = color))+geom_bar(stat = "identity", position = "dodge")+ylim(-5,5)+labs(x="Genomic coordinates (Mb)", y="Accessibility Z-score")+theme_classic(base_size = 12)+theme(legend.position = "bottom")+xlim(start,end)+geom_vline(xintercept=c(55922235/1e6,59189270/1e6), linetype = "dashed")
meth_GC
```
...
meth_CG <- ggplot(meth_bins, aes(x = (start/1e6), y= freq)) + labs(x="Genomic coordinates (Mb)", y="Methylation")+theme_classic(base_size = 12)+ylim(0,1)+geom_line(size =1)+xlim(start,end)+geom_vline(xintercept=c(55922235/1e6,59189270/1e6), linetype = "dashed")
...
top_row <- plot_grid(rep_leg, meth_CG,meth_GC, ncol =1,align = "v",rel_heights = c(1/4, 1/3, 1/3))
top_row
...
#57495001-57570000 
meth_GC <- ggplot(gc_bins, aes(x = start/1e6, y= z_score, fill = color))+geom_bar(stat = "identity", position = "dodge")+ylim(-4,5)+labs(x="Genomic coordinates (Mb)", y="Accessibility Z-score")+theme_classic(base_size = 25)+theme(legend.position = "bottom")+geom_vline(xintercept=c(57495001/1e6,57570000/1e6))+xlim(57,58)

meth_CG <- ggplot(meth_bins, aes(x = (start/1e6), y= freq)) + labs(x="Genomic coordinates (Mb)", y="Methylation")+theme_classic(base_size = 25)+ylim(0,1)+geom_line(size =1)+geom_vline(xintercept=c(57495001/1e6,57570000/1e6))+xlim(57,58)

top_row <- plot_grid(meth_CG,meth_GC, ncol =1,align = "v")
top_row

#ggsave(
#  paste0(figs, "/NanonomeCpG_GpC_pooled_cenx_hypo.pdf"),
#  plot = top_row,
#  scale = 1,
#  width = 10,
#  height = 8
