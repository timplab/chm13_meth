...
gccalls <- read_csv(paste0(dat, "/HG002_nanonome_chrX_GpCmethylation_ByCall.csv"))
```
...
meth_bins <- meth_bins %>%
  ...
  mutate(color = case_when(z_score > 0 ~ "Accessible", 
                           TRUE ~ "Inaccessible")) 
meth_GC <- ggplot(meth_bins, aes(x = start/1e6, y= z_score, fill = color))+geom_bar(stat = "identity", position = "dodge")+ylim(-5,5)+labs(x="Genomic coordinates (Mb)", y="Accessibility Z-score")+theme_classic(base_size = 25)+theme(legend.position = "bottom")+xlim(54,56)
meth_GC
#chrX:55,644,813-55,734,235
```
...
cgcalls <- read_csv(paste0(dat, "/HG002_nanonome_chrX_CpGmethylation_ByCall.csv"))
...
meth_CG <- ggplot(meth_bins, aes(x = (start/1e6), y= freq)) + labs(x="Genomic coordinates (Mb)", y="Methylation")+theme_classic(base_size = 25)+ylim(0,1)+geom_line(size =1)+xlim(54,56)
...
#ggsave(
#  paste0(figs, "/NanonomeCpG_GpC_pooled_DXZ4_profile.pdf"),
#  plot = top_row,
#  scale = 1,
#  width = 10,
#  height = 15
...
calls.reg <- as.data.frame(gccalls) %>%
  filter(start > startr) %>%
  filter(end < endr) %>%
    group_by(qname)

call.cpg <- as.data.frame(cgcalls)%>%
  filter(start > startr) %>%
  filter(end < endr) 
...
#cpg_runs <- getRuns(call.cpg, maxGap = 20)
...
#    mutate(mod = "CpG")
 ...
 gpc.ordered <- order_reads(runs)
  gpc <-  gpc.ordered$x %>%
    mutate(m = ifelse(values == 1, "Open","Closed")) %>%
    mutate(mod = "GpC")

  pal <- pal_npg("nrc")(10)
meth_pal <- c(pal[5],pal[4]) 

g <- ggplot(gpc, aes(xmin = start, xmax = end, ymin = ymin, ymax = ymax)) + geom_rect(data = gpc.ordered$bounds, fill = "grey80")+ geom_rect(aes(fill = m)) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), panel.spacing = unit(2, "lines"), legend.position = "bottom")+scale_fill_manual(name = "State", values = meth_pal)+coord_cartesian(xlim=(c(startr-500,endr+500)))+geom_vline(xintercept = (startr+endr)/2)
print(g)
