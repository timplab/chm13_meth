...
unmapped <- read_tsv("/kyber/Data/Nanopore/Analysis/gmoney/CHM13/v1.0_final_assembly/chm13_final_beds/chm13_v1.0_unmapped_byGRCh38.bed", col_names = F) %>%
  dplyr::rename("seqnames"=X1, "start"=X2, "end"=X3) %>%
  filter(seqnames == chrom)
subfam <- read_tsv("/kyber/Data/Nanopore/Analysis/gmoney/CHM13/v1.0_final_assembly/annotations/hsat/chm13.hsat3.final.strand.subfams.bed", col_names = F) %>%
  dplyr::rename("seqnames"=X1, "start"=X2, "end"=X3) %>%
  filter(seqnames == chrom)
...
subfam.sub <- subfam %>%
  filter(seqnames == chrom)
sub.plot <- ggplot(data=subfam.sub, mapping=aes(xmin=(start/1e6),xmax=(end/1e6),ymin=0,ymax=.1,fill=X4))+
  geom_rect()+theme(legend.text=element_text(size=rel(1)))+theme(legend.title=element_blank()) +theme(axis.text.y=element_blank(), axis.ticks.y=element_blank())+theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))+coord_cartesian(xlim=c(rstart/1e6,rend/1e6))+theme(legend.title=element_blank(),panel.border=element_blank())+theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(),panel.border=element_blank(), legend.position = "none")
...
top_row <- plot_grid(meth,cov.plot,sub.plot,map.plot,censat.plot,ncol = 1, align="v",rel_heights = c(1/6, 1/6,1/25,1/25,1/20))

top_row
ggsave(
  paste0(figs, "/methyl_profiles/", chrom,"_PanelPlotHSAT3sub.pdf"),
  plot = top_row,
  scale = 1,
  width = 10,
  height = 3
