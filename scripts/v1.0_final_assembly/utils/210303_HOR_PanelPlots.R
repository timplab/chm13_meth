...
cen <- read_tsv(paste0(dat, "/annotations/t2t_cenRegions.v2.021621.bed"), col_names = c("chr", "start", "end","name")) %>%
  ...
  meth <- ggplot(all.dat, aes(x = (start/1e6), y= smooth))+geom_line(size =1) + labs(y="Methylation")+theme_classic(base_size = 10)+xlim((rstart/1e6),(rend/1e6))+theme(axis.title.x=element_blank(),axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.line.x = element_blank())+theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))+ylim(0,1)+geom_vline(xintercept = c(26.250,26.350,24.9,28.3))
  ...
  top_row <- plot_grid(meth,hor.plot,censat.plot,ncol = 1, align="v",rel_heights = c(1/3,1/25,1/20))
  top_row
  ggsave(
    paste0(figs, "/methyl_profiles/", chrom,"_HOR_PanelPlotv2.pdf"),
    plot = top_row,
    scale = 1,
    width = 10,
    height = 3
