...
violin <- ggplot(cpg, aes(x = factor(name), y = CpG, fill = name))+ scale_fill_manual(values = censatColors, drop = FALSE)+geom_violin()+theme_classic(base_size = 20)+labs(x = "Repeat", y = "CpG Density")+geom_boxplot(outlier.shape = NA, width=.1)+coord_cartesian(y=c(0,30))+geom_hline(yintercept = 3, linetype= "dashed") 
 ...
 chm13_meth.lab <- as.data.frame(chm13_meth) %>%
   mutate(auto = case_when(seqnames == "chrX" ~ "chrX", 
                           TRUE ~ "autosome"))
...
violin <- ggplot(chm13_meth.lab, aes(x = auto, y = methylated_frequency, fill = auto))+geom_violin(adjust=2)+theme_classic(base_size = 20)+labs(x = "Repeat", y = "CpG Density")+geom_boxplot(outlier.shape = NA, width=.1)
  

ggsave(
  paste0(figs, "/chm13WG_methylationFreq.pdf"),
  plot = violin,
  scale = 1,
  width = 5,
  height = 5,
...
violin2 <- ggplot(HG002_meth, aes(x = auto, y = methylated_frequency, fill = auto))+geom_violin(adjust=2)+theme_classic(base_size = 20)+labs(x = "Repeat", y = "CpG Density")+geom_boxplot(outlier.shape = NA, width=.1)

ggsave(
  paste0(figs, "/HG002WG_methylationFreq.pdf"),
  plot = violin2,
  scale = 1,
  width = 5,
  height = 5,
