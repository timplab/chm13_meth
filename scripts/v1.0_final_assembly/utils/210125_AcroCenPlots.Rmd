...
CalculateCG <- function (obj, ..., view.width, as.prob = TRUE) 
{
    ...
    if (missing(view.width)) {
        ...
        res <- lapply(seqs, function(x) letterFrequencyInSlidingView(x, 
            view.width, letters = "CG", as.prob = as.prob))
        if (length(res) == 1) 
            res <- unlist(res)
    }
    res
  ...
  blocks <- genomeBlocks(BSgenome.t2t.v1.0.release, chrs = seqnames(), width = 10000)
