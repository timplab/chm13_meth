...
seqlevels(chr.meth) <- seqlevelsInUse(chr.meth)

    
  
  blocks <- genomeBlocks(BSgenome.t2t.v1.0.release, chrs = chr, width = 200)
...
meth_bins <-merge(binned_meth,binned_unmeth, by=c("seqnames", "start", "end")) %>%
    filter(seqnames == chr) %>%
    filter(start > rstart) %>%
    filter(end < rend) %>%
    group_by(start, end) %>%
    ...
    ungroup() %>%
    mutate(smooth = rollmean(freq, 15, fill = NA)) 
...
cov <- ggplot(meth_bins, aes(x = (start/1e6), y= site_smooth))+geom_line(size =1)+ labs(x="Genomic coordinates (Mb)", y="CpG Density")+theme_classic(base_size = 25)+xlim(rstart/1e6,rend/1e6)
...
top_row <- plot_grid(rep_leg2,meth,cov, ncol = 1, align="v", rel_heights = c(1/4, 1/2, 1/2))
top_row


#ggsave(
#  paste0(figs, "/methyl_profiles/", chr, "_HOR_methyl_pattern.pdf"),
#  plot = top_row,
#  scale = 1,
#  width = 10,
#  height = 12
