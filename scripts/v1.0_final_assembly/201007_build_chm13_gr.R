...
chm13_CG <- CalculateCpG(BSgenome.t2t.v1.0.release, blocks, as.prob = F)

chm13 <- GRanges(blocks, CpG = chm13_CG)
saveRDS(chm13, file =paste0(dat, "/reference/chm13_step_200_CpG_num.rds"))
#chm13 <- readRDS(paste0(dat, "/ref/chm13_step_200_CpG_freq.rds"))

freq = read_tsv(paste0(dat, "/methylation_calls/CpGmethylation50kb.freq"), col_names = F)
bis <- read.bismark(paste0(dat, "/methylation_calls/CpGmethylation50kb.freq"))
cov <- getCoverage(bis)
