...
#cpg_raw <- getRuns(reg, maxGap = 20)
#raw.ordered <- order_reads(cpg_raw)
#cpg_raw <- raw.ordered$x %>%
...
runs_plot <- runs %>%
    mutate(m = ifelse(values == 1, "Methylated","Unmethylated"))%>%
  mutate(dip = case_when(122650000 <= start & 123000000 >= end ~ "yes",
                   TRUE ~ "no"))
...
cpg_runs_reg <- cgcalls %>%
  filter(start > 121868217) %>%
  filter(end < 121897217)



calls.reg <- cpg_runs_reg %>%
    group_by(qname)
group_names <- group_keys(calls.reg)$qname
...
names(calls.list) <- group_names
smooth.list <- lapply(calls.list,smoothCalls, bandwidth = 5)
calls.smooth <- bind_rows(smooth.list,.id = "qname")
cgsmooth.list <- calls.smooth
runs <- getRuns(cgsmooth.list)
...
cpg_runs.ordered <- order_reads(runs)
  ...
  cpg_runs_reg <- cpg_runs.ordered$x %>%
    ...
    mutate(mod = "CpG")
 pal <- pal_npg("nrc")(10)
meth_pal <- c(pal[8],pal[2]) 
smooth <- ggplot(cpg_runs_reg,aes(xmin = start, xmax = end, ymin = ymin, ymax = ymax)) + geom_rect(data = cpg_runs.ordered$bounds, fill = "grey80") + 
...
meth <- cpg_runs_reg %>%
  filter(m == "Methylated")
d <- ggplot(meth, aes(width, alpha = .3))+geom_density(adjust = 3)+theme_classic()+xlim(0,50)
...
#  ggsave(
#  paste0(figs, "/", chr, "_HOR_unsmooth_singleread.pdf"),
#  plot = unsmooth,
#  scale = 1,
#  width = 10,
#  height = 10,
...
#  
# ggsave(
#  paste0(figs, "/", chr, "_meth_HOR_smooth_singleread.pdf"),
#  plot = smooth,
#  scale = 1,
#  width = 10,
#  height = 10,
...
cg_reg <- reg  %>%
  filter(start > 122712217) %>%
  filter(end < 122747217)
