...
reg <- proseq %>%
  dplyr::filter(X1 == chr) %>%
  dplyr::filter(X2 >= region_start) %>%
  dplyr::filter(X3 <= region_end) %>%
  mutate(ymin=0)

pro <- ggplot(reg, aes(xmin = X2, xmax = X3, ymin = ymin, ymax = X4))+geom_rect()+ theme(axis.text.y = element_blank(),  legend.position= "bottom",axis.ticks.y = element_blank(), panel.spacing = unit(2, "lines"))

top_row <- plot_grid(pro, meth,g, align="v", ncol=1,rel_heights = c(1/4, 1/4,1/2))
top_row
  
  ggsave(
  paste0(figs, "/", chr, "HSAT2_singleread_proseq.pdf"),
  plot = g,
  scale = 1,
  width = 10,
  height = 6,
...
```
```{r Bsat}
region_start=128299816
region_end= 128404953
reg = cgcalls %>%
  dplyr::filter(start >= region_start) %>%
  dplyr::filter(end <= region_end)
...
pro <- ggplot(reg, aes(xmin = X2, xmax = X3, ymin = ymin, ymax = X4))+geom_rect()+ theme(legend.position= "bottom", panel.spacing = unit(2, "lines"))

top_row <- plot_grid(pro, meth,g, align="v", ncol=1,rel_heights = c(1/4, 1/4,1/2))
top_row
  
  ggsave(
  paste0(figs, "/", chr, "BSAT_singleread_proseq.pdf"),
  plot = top_row,
  scale = 1,
  width = 10,
  height = 10,
