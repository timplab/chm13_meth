...
rm <- read_tsv(paste0(dat, "/annotations/chm13.draft_v1.0.fasta_rm.bed"), col_names = F) %>%
  dplyr::filter(X8 == "L1") %>%
  mutate(name = ifelse(grepl("L1M", X4), "L1M", X4)) %>%
  mutate(name = ifelse(grepl("L1P", X4), "L1P", name)) %>%
  mutate(name = ifelse(grepl("HAL", X4), "HAL", name)) %>%
...
ggsave(
  paste0(figs, "/","L1_all_quartiles_meta.pdf"),
  plot = p,
  scale = 1,
  width = 10,
  height = 5,
...
p1 <- ggplot(perc, aes(x = name,y =percent,fill = as.factor(quartile)))+geom_bar(stat = "identity")+theme_classic()

ggsave(
  paste0(figs, "/","L1_percent_quartiles.pdf"),
  plot = p1,
  scale = 1,
  width = 5,
  height = 5,
...
```{r}
violin <- ggplot(data = meth_pro_L1, aes(y = factor(quartile), x = methylated_frequency, fill = name))+geom_density_ridges(rel_min_height = 0.01,scale = 0.8,quantile_lines = TRUE, quantiles = 2, bandwidth = .05)+theme_classic(base_size = 20)+labs(x = "Methylation Frequency", y = "Transcription Quartile")+facet_wrap(~name)

ggsave(
  paste0(figs, "/","L1_quartiles_ggridges.pdf"),
  plot = violin,
  scale = 1,
  width = 10,
  height = 10,
