...
hist <- ggplot(frac.active, aes(x=perc_sub, y=frac))+geom_histogram(stat="identity", position = "dodge")+coord_flip()+xlim(0,30)
```
...
ggsave(
  paste0(figs, "/evol_meth/TE/","L1_all_quartiles_meta_chm13_chrX.pdf"),
  plot = p,
  scale = 1,
  width = 10,
  height = 10,
...
hist <- ggplot(frac.active, aes(x=perc_sub, y=frac))+geom_histogram(stat="identity", position = "dodge")+coord_flip()+xlim(0,35)
```
...
ggsave(
  paste0(figs, "/evol_meth/TE/","L1_methAge_heatmapSmoothchm13.pdf"),
  plot = last_plot(),
  scale = 1,
  width = 15,
  height = 10,
