...
ovl <- findOverlaps(r1.gr, l1_regions)
genes.ovl <- as.data.frame(rm)[subjectHits(ovl),] %>%
  dplyr::mutate(genewidth = end - start) %>%
  dplyr::rename(gene_start = start, gene_end = end) 

chm13.ovl <- as.data.frame(r1.gr[queryHits(ovl),]) %>%
  bind_cols(genes.ovl) %>%
...
chm13.ovl.labs <- chm13.ovl %>%
 # mutate(reg = ifelse(Name %in% CT.genes$Name, "CT", "non-CT")) %>%
  group_by(dist) %>%
 # summarise(med_meth = median(cov)) %>%
  distinct()

p <- ggplot(chm13.ovl.labs,aes( x = dist, y = cov)) +
...
dxz4 <- tibble(c("chrX"), c(114000000), c(114300000))
colnames(dxz4) <- c("chromosome", "start", "end")
xist <- tibble(c("chrX"), c(72253968), c(72286070))
colnames(xist) <- c("chromosome", "start", "end")
...
dxz4[["chromosome"]] <- factor(x = dxz4[["chromosome"]], 
                                     levels = chrom_order)
xist[["chromosome"]] <- factor(x = xist[["chromosome"]], 
                                     levels = chrom_order)
ggplot(data = chrom.sizes) + 
    # base rectangles for the chroms, with numeric value for each chrom on the x-axis
    geom_rect(aes(xmin = as.numeric(chromosome) - 0.2, 
    ...
    geom_rect(data = L1_meth, aes(xmin = as.numeric(chromosome) - 0.2, 
                                     ...
                                     ymax = end, ymin = start, fill = avgmeth)) + 
    scale_fill_gradient(low = "blue", high = "red") +
      geom_rect(data = dxz4, aes(xmin = as.numeric(chromosome) - 0.2, 
                                     xmax = as.numeric(chromosome) + 0.2, 
                                     ymax = end, ymin = start)) +scale_color_manual("orange")+
      geom_rect(data = xist, aes(xmin = as.numeric(chromosome) - 0.2, 
                                     xmax = as.numeric(chromosome) + 0.2, 
                                     ymax = end, ymin = start)) +scale_color_manual("orange")
```
