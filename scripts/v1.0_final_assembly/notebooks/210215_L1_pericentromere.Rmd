...
L1 <- as.data.frame(L1) %>%
  filter(name == "L1P") %>%
  dplyr::filter(width > 5000) %>%
  filter(start > 5000) %>%
  filter(end < 152396834-5000) %>%
...
theme(legend.background = element_rect(color = "black")) +
  theme_classic()+scale_x_continuous(breaks= c(-1,0,bodylen,bodylen + 1), labels = c(paste0("-",flankn/1e3,"kb"),"Start","End",paste0("+",flankn/1e3,"kb")))+ylim(0,1)+geom_rug(aes(x = min, y = 0), position = position_jitter(height = 0))+
...
geom_text(data = chm13.ovl %>% 
            ...
            aes(x=c(2),
                y=.2+height,
                label=paste("# repeats =",comma(n_genes))
                ), hjust=1)+coord_cartesian(ylim=c(.15,.3))
p
```
```{r}
# alpha boundaries 55922235-58746125
...
chm13.ovl.labs <- chm13.ovl %>%
 ...
 # mutate(methylated_frequency = num_meth/(num_meth+num_unmeth)) %>%
  ...
  group_by(region) %>%
  arrange(min) %>%
  mutate(med_smooth = rollmean(med, 3, NA))
...
ggsave(
  paste0(figs, "/CHM13_L1P_byRegion.pdf"),
  plot = p,
  width = 12,
  height = 7
)
