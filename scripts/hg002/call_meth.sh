...
bam=/uru/Data/Nanopore/Analysis/gmoney/chm13/hg002/fixed_alignments/asm8-primary-tig00021361_fixed.bam
out=/uru/Data/Nanopore/Analysis/gmoney/chm13/hg002/nanpolish_v13
repo=/home/isac/Software/nanopolish
ref=/uru/Data/Nanopore/Analysis/gmoney/chm13/hg002/old_alignments/asm8_primary.tig00021361.fa
base=asm8_primary.tig00021361_fixed
...
#	${repo}/nanopolish call-methylation -t 72 --reads ${fastq} --bam ${bam} --genome ${ref} > ${out}/${base}_methylation_calls.tsv
    #    ${repo}/scripts/calculate_methylation_frequency.py ${out}/${base}_methylation_calls.tsv > ${out}/${base}_methylation_frequency.tsv
        ...
        python3 /home/gmoney/repos/nanopore-methylation-utilities/mtsv2bedGraph_upperlower.py -c 1.5  -i ${out}/methylation_tq.tsv -g $ref > ${out}/${base}_methylation.tmp
